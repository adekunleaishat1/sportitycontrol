...
import {FaAngleLeft, FaAngleRight, FaFacebook, FaInstagram, FaTwitter} from 'react-icons/fa'
  ...
  return (
    ...
    <div className=' d-flex justify-content-between align-items-center tr'>
       <div className='d-flex justify-content-around align-items-center di'>
        <button className='bt'><FaAngleLeft/></button>
        <button className='bt'><FaAngleRight/></button>
       </div>
       <div className='d-flex justify-content-around align-items-center ind'>
         ...
         <button className='btn btn-dark btn-sm text-light in'><BsArrowDownCircle/>  Install App</button>
         <button className='btn btn-dark  us'><SlUser/></button>
       </div>
     </div>
     <div className='inerdiv'>
         ...
         <div className='d-flex justify-content-between align-items-center imdcon'>
            ...
            <div className='imd'>
              <Myimg className="cusimg"   im={require('./ab67706f000000021c256b4ef4a7d8fc0d635321.jpeg') }/> 
              <span  className='text-white fw-bold ms-3'>African Heat</span>
            </div>
            <div className='imd'>
              <Myimg className="cusimg"    im={require('./ab6761610000e5eb0f743bef043340ca5aac2205.jpeg') }/>
              <span className='text-white fw-bold ms-3'>Ayra starr</span>
            </div>
         </div>
      </div>
      <div>
        <h1 className='text-white ms-3 fs-3 fw-bold'>Made For Adekunle Aishat</h1>
        <div className='d-flex justify-content-between align-items-center mixd'>
        {apiData.map((item, index) => (
          <div key={index} className='mix'>
            <img className='cuimg' src={item.image} alt="" />
            <div className='mx-auto as'>
             <h2 className='text-white fs-6 mt-2 fw-bold'>{item.name}</h2>
             <p>{item.content}</p>
            </div>
          </div>
        ))}
